      ...
      <div className="w-full h-full flex flex-col items-center justify-center bg-gray-100 dark:bg-gray-800 rounded-xl transition-colors overflow-hidden relative border border-gray-200 dark:border-gray-700 group">
        {scene && scene.image ? (
          <>
            <img 
              src={scene.image} 
              alt={scene.sectionTitle?.[language] || scene.sectionTitle?.['it'] || (typeof scene.sectionTitle === 'string' ? scene.sectionTitle : '') || "Illustrazione scena"} 
              className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 cursor-pointer"
              onClick={() => setIsLightboxOpen(true)}
            />
            {/* Sovra-impressione che invita al click */}
      ...
      {isLightboxOpen && scene?.image && (
        ...
        >
          ...
          <button 
            ...
            onClick={(e) => {
              e.stopPropagation(); // Evita che il click si propaghi al contenitore
          ...
          <img 
            src={scene.image} 
            alt={scene.sectionTitle?.[language] || scene.sectionTitle?.['it'] || (typeof scene.sectionTitle === 'string' ? scene.sectionTitle : '') || "Illustrazione ingrandita"} 
            className="max-w-full max-h-full object-contain rounded shadow-2xl"
            onClick={(e) => e.stopPropagation()} // Cliccare sull'immagine non chiude il modale
          />
        </div>
      )}
    </>
